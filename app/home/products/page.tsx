...
export default function ProductsPage() {
  ...
  return (
    <>
      <Container>
        <div className="-mt-2 h-[85svh] overflow-hidden rounded-xl bg-muted md:-mt-4 lg:-mt-8">
          <Button
          ...
          <Card
            ...
            className={`absolute z-0 grid h-min w-[50vw] max-w-96 -translate-y-1/2 scale-75 overflow-hidden bg-opacity-10 backdrop-blur-none transition-all duration-500 ${current === 3 ? "start-1/2 top-1/2 z-10 translate-x-1/2 scale-125 shadow-xl ring-2 ring-primary [&_img]:brightness-150" : "top-1/4 w-[45vw]"} ${hidden && current === 3 ? "start-full translate-x-full scale-90 bg-transparent ring-0 -mt-10 shadow-none drop-shadow-md" : ""} ${hidden && current !== 3 ? "top-0 scale-0 opacity-0 transition-all duration-500" : ""} ${current === 1 ? "start-full translate-x-full" : ""} ${current === 2 ? "start-0" : ""} [&_img]:transition-all [&_img]:duration-500 [&_img]:hover:-skew-y-6 [&_img]:hover:skew-x-6 [&_img]:hover:scale-110`}
          >
            <CardHeader className="text-xl text-foreground sm:text-2xl md:text-4xl">
